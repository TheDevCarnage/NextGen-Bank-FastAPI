...
async def activate_user(token: str, session: AsyncSession = Depends(get_session)):
    try:
        ...
        err_msg = str(err)
        if err_msg == "Activation token expired":
            raise HTTPException(
                ...
                detail={
                    ...
                    "action_url": f"{settings.API_BASE_URL}{settings.API_V1_STRING}/auth/resend-activation-link",
                    "email_required": True,
                },
            )
        elif err_msg == "Invalid activation token":
            raise HTTPException(
        ...
        elif err_msg == "User already activated":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "status": "error",
        ...
        logger.error(f"Failed to activate user account: {str(err)}")
        raise HTTPException(
    ...
    try:
        ...
        if not user:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": "If an account exists with this email, Please check your email for the activation link",
                },
            )
        ...
        logger.error(f"Failed to resend activation link: {str(err)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "status": "error",
